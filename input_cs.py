...
def get_dataset_cs(path, num_epochs, batch_size):
    ...
    dataset_filtered_names = dataset.filter(lambda filename, label:
                                                tf.py_func(partial(_filter_size, size_threshold=900), 
                                                    [filename], 
                                                    [tf.bool]))
    ...
    dataset = dataset.map(lambda image_resized, label:
                            (tf.scalar_mul(1.0/255.0, image_resized), label))
...
def _parse_function(filename, label, im_size):
    ...
    image_resized = tf.image.resize_images(image_decoded, im_size)
